...
def extract_excel_content(file_path: str, ext: str) -> str:
    try:
        # Limit to 100 rows check
        if ext == ".csv":
            df = pd.read_csv(file_path, nrows=101)
        else:
            df = pd.read_excel(file_path, nrows=101)
        
        if len(df) > 100:
             return "System Notice: The Excel/CSV file has more than 100 rows. Please upload a file with 100 or fewer rows for processing."
