...
FAILED_VIN_SET_KEY = "analytics:failed_vins"

def log_failed_vin(whatsapp_id: str, vin: str) -> None:
    """
    Log a failed VIN search to a Redis list, keeping only the last 100 to prevent memory bloat.
    """
    try:
        # Store as JSON string {"user": whatsapp_id, "vin": vin, "time": timestamp}
        entry = json.dumps({
            "whatsapp_id": whatsapp_id,
            "vin": vin,
            "timestamp": int(time.time())
        })
        redis_client.lpush(FAILED_VIN_SET_KEY, entry)
        # Keep only the latest 100 failed searches
        redis_client.ltrim(FAILED_VIN_SET_KEY, 0, 99)
    except Exception as e:
        print(f"⚠️ Failed to log failed VIN to Redis: {e}")

def get_failed_vins(limit: int = 50) -> list:
    """
    Retrieve the most recent failed VIN searches from Redis.
    """
    try:
        raw_entries = redis_client.lrange(FAILED_VIN_SET_KEY, 0, limit - 1)
        return [json.loads(entry) for entry in raw_entries]
    except Exception as e:
        print(f"⚠️ Failed to retrieve failed VINs from Redis: {e}")
        return []
